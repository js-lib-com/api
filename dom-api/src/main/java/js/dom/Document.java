 ...
 * <p>
 * Where reasonable, search methods has variant for name space. As stated not all search methods have support for name
 * space; for example searching by CSS class has not name space since <code>class</code> attribute name is only in
 * global scope. All methods handling name spaces follows W3C DOM convention, i.e. has the name suffixed with
 * <code>NS</code>. First parameter is always name space. If name space parameter is null, name space aware methods
 * degenerate to their not name space counterpart, e.g. <code>getByTagNS(null, "p")</code> delegates
 * <code>getByTag("p")</code>.
 * <p>
 * If document is parsed without name space support, element 'NS' getters always returns null and 'NS' finders always
 * return empty list. Also, on document without name space support, element and attribute names include prefix. For
 * example, <code>ns:name</code> element has null name space and local name 'ns:name'. When searching for element use
...
public interface Document
{
   ...
   * space. Note that newly created element is not part of document tree until explicitly add or insert it as a child to
   * a parent. So, elements creation follows the same W3C DOM pattern: create the element then add it as a child.
   * 
   * <pre>
   ...
   * <p>
   * This method has no attributes parameters as {@link #createElement(String, String...)} because is user code decision
   * if attribute name has name space or not.
   * 
  ...
  /**
   * Get the element with tag name in the requested name space. Tag name is a local name, that is, has no name space
   * prefix; name space if identified by separated <code>namespaceURI</code> parameter.
   * <p>
   * Search entire document for elements with given tag name, within name space, and return the first found. Returns
   * null if there is no element with requested tag name. Note that wild card asterisk (*) for tag name matches all tags
   * in which case first child is returned. Always returns null if this document is not parsed with name space support.
   * <p>
   ...
   * <code>xmlns-litteral:user-defined-prefix="namespace-uri"</code>. xmlns-litteral is the reserved keyword
   * <code>xmlns</code> whereas prefix is user defined. Namespace declaration creates a bound between prefix and
